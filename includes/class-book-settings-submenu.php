...
class Book_Settings_SubMenu {
	...
	public function book_plugin_display_fun( $args ) {
		...
		<input type="number" id="<?php echo esc_attr( $args['label_for'] ); ?>"
			...
			value="<?php ( ! empty( $options ) && '' !== esc_attr( $options[ $args['label_for'] ] ) ) ? esc_html_e( $options[ $args['label_for'] ], 'simple-book-plugin' ) : '10'; ?>">
		books
		<?php
	}
	 ...
	 *
	 * @param array $args Arguments for display post types.
	 *
	 * @return void
	 */
	public function book_plugin_posts_fun( $args ) {
		...
		$countries_json = wp_remote_get( dirname( WPBOOK_PLUGIN_FILE ) . '/json/countries.json' );
		...
		<select id="<?php echo esc_attr( $args['label_for'] ); ?>"
			...
			foreach ( $countries['countries'] as $country ) {
				...
				?>
				<option <?php echo $selected; ?> value="<?php echo esc_html_e( $country['countryName'] . ' - ' . $country['currencyCode'] ); ?>"><?php echo esc_html_e( $country['countryName'] . ' - ' . $country['currencyCode'] ); ?></option>
				<?php
			}
			?>
		</select>
