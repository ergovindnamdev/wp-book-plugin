...
class WPBook_Shortcode
{
    ...
    public function book_shortcode_fun($atts, $content = "")
    {
        ...
        /**
         * If need to fetch from post meta
         */
        // $args = array_merge(
        //     $args,
        //     array(
        ...
        if (!empty($bookids)) {
            $bookIds = explode(",", implode(", ", $bookids) . ', ' . $atts['id']);
        } else {
            $bookIds = explode(",", $atts['id']);
        }

        if (!empty($bookIds)) {
            $args = array_merge($args, array("post__in" => $bookIds));
        }
    ...
    public function get_bookids_from_meta($author_name, $publisher, $year)
    {
        ...
        $bookIds = $wpdb->get_col($wpdb->prepare('SELECT book_id FROM ' . $wpdb->prefix . 'book_meta WHERE 1=1 ' . $where, 1));
        return $bookIds;
    }
}
